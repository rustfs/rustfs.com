  ...
  if (!platform.available) {
    return (
      <div className={cn("text-center py-12", className)}>
        ...
        <p className="text-muted-foreground mb-4">{tw("即将开放下载", "Coming Soon", "Yakında")}</p>
        <div className="max-w-md mx-auto p-4 bg-muted/30 rounded-lg border border-dashed border-muted-foreground/30">
          <p className="text-sm text-muted-foreground">
            {tw("我们正在为此平台准备 RustFS，敬请期待！", "We are preparing RustFS for this platform, stay tuned!", "Bu platform için RustFS hazırlıyoruz, bizi takip edin!")}
          </p>
        </div>
      </div>
    );
    ...
    <div className={cn("space-y-8", className)}>
      ...
      {platform.id === "docker" && platform.variants && (
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            <h3 className="text-lg font-semibold text-foreground">{tw("Docker 部署", "Docker Deployment", "Docker Dağıtımı")}</h3>
            <a
              ...
              <span>{tw("查看镜像", "View Image", "İmajı Görüntüle")}</span>
            ...
            <div key={index} className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {tw("版本", "Version", "Sürüm")}: {variant.architecture}
                  </p>
                </div>
                <a
                  ...
                  <span>{tw("查看", "View", "Görüntüle")}</span>
                </a>
              </div>

              <CodeBlock code={variant.commands} title={tw("Docker 命令", "Docker Commands", "Docker Komutları")} />
      ...
      {platform.variants && platform.variants.length > 0 && platform.id !== "docker" && (
        <div className="space-y-6">
          <h3 className="text-lg font-semibold text-foreground">{tw("二进制下载", "Binary Downloads", "İkili Dosya İndirmeleri")}</h3>
            ...
            <div key={index} className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-medium text-foreground">{variant.name}</h4>
                  <p className="text-sm text-muted-foreground">
                    {tw("架构", "Architecture", "Mimari")}: {variant.architecture}
                  </p>
                </div>
                <a
                  href={variant.downloadUrl}
                  ...
                  <span>{tw("下载", "Download", "İndir")}</span>
                </a>
              </div>

              <CodeBlock code={variant.commands} title={tw("安装命令", "Installation Commands", "Kurulum Komutları")} />
      ...
      {platform.installScript && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-foreground">{tw("一键安装", "One-click Installation", "Tek Tıkla Kurulum")}</h3>

          <CodeBlock code={[platform.installScript]} title={tw("Shell 脚本", "Shell Script", "Shell Betiği")} />
        </div>
      )}
      ...
      {platform.brewConfig && platform.brewConfig.length > 0 && platform.id == "macos" && (
        <div className="space-y-6">
          <h3 className="text-lg font-semibold text-foreground">
            {tw("Homebrew安装（建议）", "Via Homebrew (Recommended)", "Homebrew ile (Önerilen)")}
          </h3>
