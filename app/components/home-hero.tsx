...
export default function HomeHero() {
  ...
  return (
    <section className="mx-auto max-w-7xl overflow-hidden px-4 sm:px-6 lg:px-8 pt-20 pb-16 text-center lg:pt-32">
      <h1 className="mx-auto font-display text-4xl font-extrabold tracking-wide text-primary sm:text-7xl">
        {tw('Rust 驱动的', 'Rust-powered')}{/* */}{" "}
        <span className={cn('relative whitespace-nowrap text-blue-600', locale == 'en' ? 'flex justify-center mx-auto' : 'inline-flex')}>
          <svg
            aria-hidden="true"
            viewBox="0 0 418 42"
            className={cn("absolute top-2/3 mx-auto max-w-[50vw] h-[0.58em] fill-blue-300/70", locale == 'en' ? 'w-auto' : 'w-full')}
          ...
          <span className={cn('relative flex')}>
            <WordRotate words={[tw('高性能', 'High Performance'), tw('无限扩容', 'Unlimited Scaling'), tw('安全可靠', 'Secure and Reliable'), tw('多云存储', 'Multi-cloud Storage'), tw('S3 兼容', 'S3 Compatible')]} className="min-w-[400px] pl-8 text-left" />
          </span>
        </span>{" "}
        {/* */} {locale !== 'en' && <br />} {tw('分布式存储系统', 'Distributed Storage System')}
      </h1>
