    ...
    function mainMdxLoader(plugins) {
      return [
        ...
        createLoader(function (source) {
          ...
          let { attributes: meta, body } = frontMatter<Record<string, any>>(source)

          // if no published date, use the file's mtime
          if (!meta['published']) {
            meta.published = fs.statSync(this.resourcePath).mtime.toISOString()
          }

          // if no title, use the h1 from the markdown
          if (!meta['title']) {
            let match = body.match(/^#\s+(.*)$/m)
            meta.title = match ? match[1] : ''
          }

          // new RegExp(RegExp.escape(basePath))
          const basePath = this.resourcePath.match(/src\/pages\/(.*)\.mdx$/)?.[1]
          meta.match = new RegExp(basePath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
          meta.href = basePath

          if (fields) {
            for (let field in (meta as Record<string, any>)) {
